...
**現場觀察 #1(2026-04-26):「新會議預約」按鈕引發的語義衝突**

實戰場景:MX IP計畫(專案 MTRF1H)累積了 4/1 + 4/24 兩場會議日誌,qmore 預約 4/29 會議時直覺認為「會議日誌應該被清空」(因為下一場是新一輪)。但這個直覺和「日誌是專案歷史」的另一個直覺衝突 — 同個專案的多次會議,日誌本來就該疊加。

這個衝突暴露了 ❶ 的核心矛盾:**「新會議預約」這個按鈕的語義到底是什麼**?

| 語義解讀 | 行為 | 對應的設計哲學 |
|---|---|---|
| 「在這個主題下開下一場會」 | 更新 meeting_date,日誌保留為主題歷史 | **哲學 B**(一專案多會議) |
| 「結束這專案,從零開始」 | 清提案 + 清日誌(像 reset) | **哲學 A**(一專案一會議) |
| 「先暫停舊主題,開個新主題」 | 不該用同一個 6 碼,應該建新專案 | **哲學 A 的另一種展開** |

**目前的實作矛盾:** `openNewRound`(`index.html` line 1689)目前**會清提案但不清日誌** — 這是兩種哲學的混血,沒有一個一致的心智模型可以解釋。

**Phase 3 必須處理的具體決定:**

1. ⏭ 「新會議預約」這個按鈕是要保留還是拆成兩個動作?
   - 拆 A:「⚙ 改開會時間」(只改日期,什麼都不清)+「✕ 結束此專案」(獨立按鈕,真的要重置才用)
   - 拆 B:整個「新會議預約」拿掉,改在會議列表新增「+ 開下一場」(需要 `meetings` 表)
2. ⏭ 「📚 引用過去日誌」的範圍 — 是同一專案內的還是跨專案?
3. ⏭ 「📌 上次會議備忘」的 source — 是「同個專案上一場」還是「使用者上一場任何會議」?

**短期決議(2026-04-26):** 不動目前邏輯。「新會議預約」維持「更新日期 + 清提案,日誌保留」的混血行為,等 Phase 3 一起決定。

**為什麼不在 Phase 2.5 處理:** 任何修改都會牽動 `proposals.project_id` 和 `journals.project_id` 的語義 — 是綁專案還是綁會議?Phase 3 不決定哲學就修,只會多累積一層債。
